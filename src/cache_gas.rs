use alloy::{
    eips::BlockId,
    network::Network,
    primitives::{Address, Bytes, StorageKey, StorageValue, TxHash, B256, U128, U256},
    providers::{
        ParamsWithBlock, Provider, ProviderCall, ProviderLayer, RootProvider, RpcWithBlock,
    },
};
use alloy_json_rpc::{RpcError, RpcSend};
use alloy_primitives::keccak256;
use alloy_rpc_types::{BlockNumberOrTag, EIP1186AccountProofResponse, Filter, Log};
...
impl CacheLayer {
    ...
    pub fn new(max_items: u32) -> Self {
        Self {
            cache: SharedCache::new(max_items),
        }
    }
    ...
    N: Network,
{
    ...
    // -------------------------------------------------------------------------
    // NEW METHOD: get_gas_price
    // -------------------------------------------------------------------------
    //
    // This is similar to `get_logs` in that it is an async method returning a
    // TransportResult. We perform the cache check first, then fall back to the
    // RPC if there is a cache miss.
    //
    // Note that eth_gasPrice takes no parameters, so we pass () to `RequestType`.
    // Since the method is always the same with no params, it will always map to
    // the same cache key. If you desire more frequent updates, you may want to
    // skip caching or refresh older entries.
    fn get_gas_price(&self) -> ProviderCall<[(); 0], U128, u128> {
        let req = RequestType::new("eth_gasPrice", ());
        ...
        ProviderCall::BoxedFuture(Box::pin(async move {
            ...
            if let Some(hash) = params_hash {
                if let Ok(Some(cached)) = cache.get_deserialized_with_timestamp::<U128>(&hash) {
                    // Only use cached value if it's less than 15 seconds old
                    if (chrono::Utc::now() - cached.timestamp).num_seconds() < 15 {
                        return Ok(cached.value.to::<u128>());
                    }
                }
            }

            // Cache miss or expired entry: call the underlying RPC
            let client = client
                .upgrade()
                .ok_or_else(|| TransportErrorKind::custom_str("RPC client dropped"))?;
            ...
            if let Some(hash) = params_hash {
                let _ = cache.put_with_timestamp(hash, result);
            }
...
struct FsCacheEntry {
    ...
    value: String,
    /// Optional timestamp when this entry was cached (in seconds since UNIX epoch)
    /// Only used for time-sensitive entries like gas price
...
pub struct SharedCache {
    ...
    // Track timestamps separately since Instant isn't Serialize/Deserialize compatible
    timestamps: Arc<RwLock<std::collections::HashMap<B256, chrono::DateTime<chrono::Utc>>>>,
...
impl SharedCache {
    ...
    pub fn new(max_items: u32) -> Self {
        ...
        let inner = Arc::new(RwLock::new(LruCache::with_hasher(
            max_items,
            Default::default(),
        )));
        let timestamps = Arc::new(RwLock::new(std::collections::HashMap::new()));
        Self {
            inner,
            timestamps,
            max_items,
    ...
    {
        let Some(cached) = self.get(key) else {
            return Ok(None);
        };
    ...
    /// Puts a value with timestamp into the cache
    pub fn put_with_timestamp<T: Serialize>(&self, key: B256, value: T) -> TransportResult<bool> {
        let json_str = serde_json::to_string(&value).map_err(TransportErrorKind::custom)?;
        let result = self.inner.write().put(key, json_str).is_some();

        // Store the current timestamp
        self.timestamps.write().insert(key, chrono::Utc::now());

        Ok(result)
    }

    /// Get deserialized value with timestamp from the cache
    pub fn get_deserialized_with_timestamp<T>(
        &self,
        key: &B256,
    ) -> TransportResult<Option<TimestampedValue<T>>>
    where
        T: for<'de> Deserialize<'de>,
    {
        let Some(cached) = self.get(key) else {
        ...
        let value = serde_json::from_str(&cached).map_err(TransportErrorKind::custom)?;
        Ok(Some(TimestampedValue { value, timestamp }))
    }
    ...
    pub fn save_cache(&self, path: PathBuf) -> TransportResult<()> {
        let entries: Vec<FsCacheEntry> = {
            ...
            cache
                .iter()
                .map(|(key, value)| {
                    // Convert Instant to UNIX timestamp for serializable entries
                    let timestamp = timestamps.get(key).map(|_instant| {
                        // This is an approximation since we can't get the exact UNIX time from Instant
                        // For our purposes (expiring cache entries), this is sufficient
                        chrono::Utc::now()
                    });
    ...
    pub fn load_cache(&self, path: PathBuf) -> TransportResult<()> {
        ...
        for entry in entries {
            ...
            // If we have a timestamp, reconstruct an approximate Instant
            // This won't be exact but is good enough for cache expiration
            if entry.timestamp.is_some() {
                timestamps.insert(entry.key, now);
            }
        }
...
//     use alloynetwork::TransactionBuilder;
